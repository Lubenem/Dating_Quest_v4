...
import { Colors, ActionColors } from '../app/constants';
...
export default function ReactNativeMapScreen() {
  ...
  const offsetLocation = (
    ...
    switch (direction) {
      case 'north':
        return { latitude: lat + offset, longitude: lng };
      case 'east':
        return { latitude: lat, longitude: lng + offset };
      case 'south':
        return { latitude: lat - offset, longitude: lng };
      case 'west':
        return { latitude: lat, longitude: lng - offset };
    ...
    return (
      <SafeAreaView style={styles.container}>
        <View style={styles.errorContainer}>
          ...
          <Text style={styles.errorMessage}>Please enable location permissions to view the map.</Text>
        ...
        {ENABLE_TEST_MARKERS && (
          <>
            <Circle
              ...
              strokeWidth={2}
            />
            <Marker coordinate={offsetLocation(userLocation.latitude, userLocation.longitude, 'east')} tracksViewChanges={tracksViewChanges}>
            ...
            <Marker coordinate={offsetLocation(userLocation.latitude, userLocation.longitude, 'south')} tracksViewChanges={tracksViewChanges}>
              <View style={styles.markerContainer} onLayout={handleMarkerLayout}>
            ...
            <Marker coordinate={offsetLocation(userLocation.latitude, userLocation.longitude, 'west')} tracksViewChanges={tracksViewChanges}>
              <View style={styles.markerContainer} onLayout={handleMarkerLayout}>
...
const styles = StyleSheet.create({
  ...
  errorTitle: { fontSize: 24, fontWeight: 'bold', color: Colors.text, marginBottom: 16, textAlign: 'center' },
  errorMessage: { fontSize: 16, color: Colors.textSecondary, textAlign: 'center', marginBottom: 12, lineHeight: 24 },
  errorDetail: { fontSize: 14, color: Colors.error, textAlign: 'center', marginTop: 8 },
});
